...
-(void)insertDataModel:(NewsModel *)model
{
    if([_database open]){
        ...
        BOOL isSuccess = [_database executeUpdate:sql, model.myId, model.title,model.comment_count,model.source,model.icon,model.myShort];
        if(!isSuccess){
...
-(NSArray *)findAll
{
    ...
    if([_database open]){
        ...
        while ([set next]) {
            ...
            model.myShort = [set stringForColumn:@"short"];
...
- (void)deletDataModel: (NSString *)newsId
{
    ...
    if ([_database open]) {
        ...
        if (!isSuccess) {
            NSLog(@"delete erroe = %@",_database.lastErrorMessage);
        }
